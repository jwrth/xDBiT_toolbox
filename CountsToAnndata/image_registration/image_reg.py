...
assert np.all([pd.notnull(a) == pd.notnull(b) for a, b in zip(directories["align_images"], directories["hq_images"])]), \
    "For some datasets only alignment images OR hq images were given. Either both are given or None. If you wish to use the alignment images as hq images input the alignment directories also in the hq image section. If you wish to create an anndata without image data leave both sections empty."
